...
class PPOMemory:
    ...
    def generate_batches(self):
        ...
        # Convert states to a list of tensors
        state_tensors = [T.tensor(state, dtype=T.float) for state in self.states]
        
        # Pad sequences to ensure they all have the same length
        padded_states = pad_sequence(state_tensors, batch_first=True).numpy()
...
class SelfAttention(nn.Module):
    def __init__(self, input_size, embed_size=128, num_heads=config.num_heads, mask=mach_mask):
...
class ActorNetwork(nn.Module):
    def __init__(self, input_size, dim_k=128, dim_v=128, num_heads=config.num_heads, chkpt_dir=os.path.join(experi_dir(), 'param')):
...
class Agent:
    def __init__(self, input_size, gamma=0.99, alpha=0.00001, gae_lambda=0.98, policy_clip=0.2, batch_size=5, n_epochs=10):
        self.gamma = gamma
